...
void BFSSolver(Graph &g, int source, DistT *h_dists) 
{
  ...
  cudaMalloc((void **)&d_row_offsets, (m + 1) * sizeof(uint64_t));
  cudaMalloc((void **)&d_column_indices, nnz * sizeof(VertexId));
  cudaMemcpy(d_row_offsets, h_row_offsets, (m + 1) * sizeof(uint64_t), cudaMemcpyHostToDevice);
  cudaMemcpy(d_column_indices, h_column_indices, nnz * sizeof(VertexId), cudaMemcpyHostToDevice);

  DistT zero = 0;
  DistT * d_dists;
  cudaMalloc((void **)&d_dists, m * sizeof(DistT));
  cudaMemcpy(d_dists, h_dists, m * sizeof(DistT), cudaMemcpyHostToDevice);
  cudaMemcpy(&d_dists[source], &zero, sizeof(zero), cudaMemcpyHostToDevice);
  ...
  cudaMemcpy(h_dists, d_dists, m * sizeof(DistT), cudaMemcpyDeviceToHost);
