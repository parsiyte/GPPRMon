...
def plot_GV100(im, l1d, l2, dram, kernel, start, finish, l1d_rgb, l2_rgb, dram_rgb, power, arch):

  if arch == "JetsonAGXXavier" or arch == "XavierNX":  
    ...
    im.text((1120, 870),  arch + " GPU based on Volta Architecture includes " + str(len(l1d)) + " Streaming Multiprocessors (same # of L1D Caches).", font = font_info, fill=(255,255,255)) 
    im.text((1120, 895),  " There are 8 memory partitions (8 Seperate DRAM Banks) with 2 sub-partitions (16 L2 Caches).", font = font_info, fill=(255,255,255)) 
  ...
  im.text((15, 900), "Peak Dynamic (mW) : "       , font = power_data, fill=(0, 0, 0))
  im.text((15, 925), "Sub-Threshold Leakage (mW): ", font = power_data, fill=(0, 0, 0))
  im.text((15, 950), "Gate Leakage (mW) : "       , font = power_data, fill=(0, 0, 0))
  im.text((15, 975), "Run Time Dynamic (mW) : "    , font = power_data, fill=(0, 0, 0))
